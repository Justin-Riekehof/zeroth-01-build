...
Vendored from ksim-zbot (kscalelabs/ksim-zbot, MIT) `ksim_zbot/zbot2/common.py`.
Local changes for this build ("Pixel", 16 DoF, 5-DoF legs):
  * Asset paths default to sim/assets/zbot-pixel (generated by sim/tools/build_model.py).
  * get_actuators(): joints are ordered by MuJoCo joint order (nn_id) instead of servo id.
    Upstream sorted by servo id, which only works when both orders happen to coincide --
    with this build's real servo ids (hip_pitch=x1 but third joint in the kinematic chain)
    the sys-ID parameters and kp/kd would land on the WRONG joints. The physics ctrl vector
    is in MuJoCo actuator order, so that is the order the parameter arrays must follow.
    Servo ids remain in metadata.json purely as the deployment mapping.
"""
...
_ASSETS_DIR = Path(__file__).resolve().parents[1] / "assets" / "zbot-pixel"


def _install_json_logger_shim() -> None:
    """Work around a bug in the pinned xax 0.2.6 JSON logger.

    ksim 0.0.31 keeps JAX arrays in the xax training State; JsonLogger feeds
    `asdict(state)` straight into `json.dumps`, which raises TypeError and kills the
    training loop after the first logged step. Shim `json` inside that one module so
    its dumps() learns to convert arrays; everything else sees the stdlib json.
    """
    import json as _stdlib_json

    import xax.task.loggers.json as _xax_json_logger
    ...
    class _JsonShim:
        ...
        def dumps(data: object, **kwargs: object) -> str:
            ...
            return _stdlib_json.dumps(data, **kwargs)

    _xax_json_logger.json = _JsonShim()  # type: ignore[attr-defined]


_install_json_logger_shim()
...
class ZbotTask(ksim.PPOTask[Config], Generic[Config, ZbotModel]):
    ...
    def get_actuators(
        ...
        # Order by MuJoCo joint order (nn_id): the ctrl vector the physics engine consumes
        # is in MuJoCo actuator order, which in our MJCF equals joint order. Sorting by
        # servo id (upstream behavior) scrambles per-joint parameters for this build.
        sorted_joints = sorted(self.joint_mappings.items(), key=lambda x: x[1]["nn_id"])
