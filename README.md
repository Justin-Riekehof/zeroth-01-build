...
- [ ] **Restore deploy access to the Pi — blocking, next step.** The robot's `~/.ssh/authorized_keys` holds only the retired laptop's key, and that private key went with the laptop reset, so *no* machine can reach the Pi over SSH any more. There is no remote way back in — by design: `sshd` is publickey-only, the intent service exposes no file-write primitive (demo saves are slug-restricted to `demos/*.json`), and the serial console was freed for the servo UART. Route: clean shutdown → SD card into the dev PC → append the new machine's public key to `~/.ssh/authorized_keys` on the rootfs. Ship the pending Pi service **v6** (wireless joint-range calibration) and the widened shoulder limits in the same step — until then the robot clamps every taught pose to its last deployed limits (`push_ups` loses ~28° per shoulder). See [docs/pi-bringup.md](docs/pi-bringup.md).
